...
`profiles/gta-vi/media-source.json` authorizes only the official Rockstar
...
Run `R2_PUBLIC_BASE_URL=https://media.example.com bun run media:sync` to
...
`bun run media:publish` is a no-network dry run. Add `--publish` and set
