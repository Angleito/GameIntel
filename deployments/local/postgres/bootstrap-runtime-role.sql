...
-- Guard first: application logins must never be the capability group roles
-- themselves, because those roles carry direct table grants that would
-- survive any membership revocation and would otherwise be enabled as LOGIN
-- by the ALTER ROLE below. (Generated through format() + \gexec because psql
-- does not interpolate variables inside dollar-quoted DO blocks.)
SELECT format(
  $guard$
  DO $$ BEGIN
